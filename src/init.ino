...
void hardware_init()
{
  ...
  //初始化DS1302引脚
  ds_rtc.init();
  bool i=ds_rtc.isHalted();//检查运行DS1302
...
void software_init()//軟件初始化
{
 ...
 //111111 screen_loopEnabled = false;
  ...
  oledState = OLED_ON;
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0 )//ESP_SLEEP_WAKEUP_UNDEFINED) //如果是系统复位唤醒的, 则停止工作, 亮屏
  {
